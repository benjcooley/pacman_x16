; ------------------------------------------------------------
; test_scroll_visible.asm
; X16 Assembly program that demonstrates VERA scrolling with visible effect
; Keeps scroll values in reasonable range to see the movement
; ------------------------------------------------------------
...
no_h_carry:
    ...
    ; Keep horizontal scroll in range 0-319 (40 characters * 8 pixels)
    lda h_scroll_hi
    cmp #1
    bcc h_in_range
    lda h_scroll_lo
...
no_v_carry:
    ...
    ; Keep vertical scroll in range 0-239 (30 lines * 8 pixels)
    lda v_scroll_hi
    bne v_reset         ; If high byte != 0, reset
    lda v_scroll_lo
    cmp #240
    bcc v_in_range
v_reset:
