...
; Enable by default during bring-up; disable for release builds
.define ENABLE_LOGGING 1
; .define ENABLE_QUEUE_TRACE 1
; .define ENABLE_STATE_TRACE 1
; .define ENABLE_QUEUE_TRACE 1
...
; ==============================================================================
; ROM-COMPATIBLE INTRA-FRAME COMMAND QUEUE (central feature)
; ==============================================================================

; Queue layout and pointers (low-address ring buffer, 64 bytes => 32 entries)
QUEUE_SIZE          = 64                ; bytes, 2 bytes per entry (cmd,param)
queue_buf:         .res QUEUE_SIZE     ; initialized to $FF in queue_init
queue_wptr:        .res 1              ; write index (0..63), even positions preferred
queue_rptr:        .res 1              ; read index (0..63)
pac_center_consumed_flag: .res 1       ; one-shot guard per center to avoid double-consume

; Command IDs (ROM-aligned: embed number in name for auditability)
CMD_00_INIT_VIDEO                 = $00
CMD_01_GAME_STATE_TICK            = $01
CMD_02_SCREEN_DATA_BLIT           = $02
CMD_03_SCREEN_RENDER_PASS         = $03
CMD_04_LEVEL_INIT                 = $04
CMD_05_GHOST_STATE_MGMT           = $05
CMD_06_INPUT_PROCESS              = $06
CMD_07_SCORE_DISPLAY_UPDATE       = $07
CMD_08_GHOST0_OBSTACLE_AVOID      = $08   ; Blinky avoid
CMD_09_GHOST1_OBSTACLE_AVOID      = $09   ; Pinky avoid
CMD_0A_GHOST2_OBSTACLE_AVOID      = $0A   ; Inky avoid
CMD_0B_GHOST3_OBSTACLE_AVOID      = $0B   ; Clyde avoid
CMD_0C_GHOST0_SCATTER_TARGET      = $0C
CMD_0D_GHOST1_SCATTER_TARGET      = $0D
CMD_0E_GHOST2_SCATTER_TARGET      = $0E
CMD_0F_GHOST3_SCATTER_TARGET      = $0F

; Non-ROM gameplay helpers (kept out of 0x08..0x0F AI range)
CMD_12_FRUIT_CONTROL              = $12
CMD_13_PELLET_CONSUMPTION         = $13
CMD_14_POWER_PELLET_EFFECTS       = $14

; Enqueue helper (A=cmd, X=param will be used by queue_post)
.macro QUEUE_CMD cmdImm, paramImm
    lda #(cmdImm)
    ldx #(paramImm)
    jsr queue_post
.endmacro
...
@tick_done:
    ...
    ; Update game state
    jsr game_update
    ; Seed top-level state tick into queue during PLAYING
    lda game_state
    cmp #GAME_STATE_PLAYING
    bne @no_seed
    ; clear per-center consume guard and seed state tick
    stz pac_center_consumed_flag
    lda #CMD_01_GAME_STATE_TICK
    ldx #$00
    jsr queue_post
@no_seed:
    ; Drain any intra-frame queued commands (ROM-compatible FIFO)
    jsr queue_drain_all
    ; Update audio envelopes/loops
    jsr audio_update
    ; Optional per-frame state trace after drain
.ifdef ENABLE_STATE_TRACE
    ; Frame header: tick_lo, dots_remaining
    LOG_P2 210, tick_lo, dots_remaining
    ; Pac position: pacman_x, pacman_y
    LOG_P2 211, pacman_x, pacman_y
.endif
...
; ZP aliases are defined later in the file in the existing block; keep code aware
; queue_init: fill buffer with $FF, reset r/w pointers
; queue_post: enqueue (A=cmd) and (X=param) into ring (2-byte entries). If full, drop.
; queue_drain_all: drain FIFO, dispatch via jump table
...
; A = cmd, X = param
queue_post:
    ; compute next write index (wptr + 2) & 0x3F and check full
    pha                     ; save cmd
    lda queue_wptr
    clc
    adc #2
    and #$3F
    cmp queue_rptr
    beq @qp_drop_pop        ; full -> drop
    ; write cmd and param at current wptr
    ldy queue_wptr          ; Y = wptr
    pla                     ; A = cmd
    sta queue_buf,y         ; write cmd
    iny
    txa                     ; A = param
    sta queue_buf,y         ; write param
    iny
    tya
    and #$3F
    sta queue_wptr
...
; Drain all queued commands (FIFO) and dispatch
queue_drain_all:
    ldy queue_rptr
@qd_loop:
    cpy queue_wptr
    ...
    lda queue_buf,y
    sta ZP_TEMP1            ; param
    ; optional trace of (cmd,param)
.ifdef ENABLE_QUEUE_TRACE
    txa                      ; A = cmd
    sta ASM_LOG_PARAM1
    lda ZP_TEMP1             ; A = param
    sta ASM_LOG_PARAM2
    lda #201                 ; msg id for queue cmd
    sta ASM_LOG_INFO
.endif
    ; mark consumed slots as $FF
    lda #$FF
    dey
    sta queue_buf,y
    iny
    sta queue_buf,y
    ; advance rptr by 1 (currently at param), then one more to next entry
    iny
    tya
    and #$3F
    tay
    sty queue_rptr
    ; dispatch: A=cmd, ZP_TEMP1=param
    txa
    jsr queue_dispatch
    ldy queue_rptr
    bra @qd_loop
...
; queue_dispatch: A=cmd, ZP_TEMP1=param
queue_dispatch:
    and #$1F
    asl
    tax
...
; ==============================================================================
; COMMAND DISPATCH TABLE (ROM-compatible IDs → modular handlers)
;  0x01 → qh_01 (CMD_01_GAME_STATE_TICK)
;  0x06 → cmd_06_input_process
;  0x07 → cmd_07_score_display_update
;  0x08..0x0B → cmd_08..0B_ghost*_obstacle_avoid
;  0x0C..0x0F → cmd_0C..0F_ghost*_scatter_target
;  0x09 → cmd_09_fruit_control        ; per definitive Z80 code
;  0x0A → cmd_0A_pellet_consumption   ; per definitive Z80 code
; Unimplemented commands fall through qh_* stubs (rts).
; ==============================================================================
; Jumptable (default handlers are stubs initially)
queue_jumptab:
    .word qh_00, qh_01, qh_02, qh_03
    .word qh_04, qh_05, cmd_06_input_process, cmd_07_score_display_update
    .word cmd_08_ghost0_obstacle_avoid, cmd_09_ghost1_obstacle_avoid, cmd_0A_ghost2_obstacle_avoid, cmd_0B_ghost3_obstacle_avoid
    .word cmd_0C_ghost0_scatter_target, cmd_0D_ghost1_scatter_target, cmd_0E_ghost2_scatter_target, cmd_0F_ghost3_scatter_target
    .word cmd_10_blinky_targeting, cmd_11_ghost_movement_coord, cmd_09_fruit_control, cmd_0A_pellet_consumption
    .word cmd_14_level_progression, qh_15, qh_16, qh_17
    .word qh_18, qh_19, qh_1A, qh_1B
    .word qh_1C, qh_1D, qh_1E, qh_1F

; Stub handlers (to be wired up incrementally)
qh_00: rts
; ==============================================================================
; COMMAND IMPLEMENTATION: CMD_01_GAME_STATE_TICK
; Purpose: Seed per-frame subsystems in ROM-like order for gameplay state.
; ==============================================================================
qh_01:
    lda #CMD_06_INPUT_PROCESS
    ldx #$00
    jsr queue_post
    lda #CMD_07_SCORE_DISPLAY_UPDATE
    ldx #$00
    jsr queue_post
    ; Model ROM's per-frame dot/fruit management via queue commands (engine-internal IDs)
    lda #CMD_13_PELLET_CONSUMPTION
    ldx #$00
    jsr queue_post
    lda #CMD_12_FRUIT_CONTROL
    ldx #$00
    jsr queue_post
    rts
qh_02: rts
...
; Placeholders for verified ROM IDs present in jump table
cmd_10_blinky_targeting: rts
cmd_11_ghost_movement_coord: rts
cmd_14_level_progression: rts
qh_15: rts
qh_16: rts
qh_17: rts
qh_18: rts
...
; ==============================================================================
; RNG: ROM-like LCG (next = current*5 + 1) mod 8192; return a byte from memory seed
; We'll approximate the ROM behavior by returning the low byte of the seed after LCG.
; rng_seed_hi:lo holds a 13-bit value (we mask to 0x1FFF)
rng_lcg_byte:
    ; HL := seed
    lda rng_seed_lo
    sta ZP_TEMP1
    ...
    inc ZP_TEMP4
:
    ; mask to 13 bits (0x1FFF)
    lda ZP_TEMP4
    and #$1F
    sta rng_seed_hi
    lda ZP_TEMP3
    sta rng_seed_lo
    ; return low byte as pseudo-random
    lda rng_seed_lo
    rts
...
game_init:
    ...
    jsr vera_init
    lda #17                 ; "Pacman: Initializing 4bpp palette mode"
    sta ASM_LOG_INFO
    ...
    lda #11                 ; "Pacman: Setting VERA tile mode 40x31"
    sta ASM_LOG_INFO
    ...
    lda #14                 ; "Pacman: Screen layout test complete"
    sta ASM_LOG_INFO
...
update_playing:
    ; Pause toggle handled by queued input (qh_06)
    lda paused_flag
    beq @not_paused
...
@no_ready:
    ...
    ; Update player
    jsr update_player
    ; Log Pac position post-move (optional)
.ifdef ENABLE_STATE_TRACE
    LOG_P2 212, pacman_x, pacman_y
.endif
    
    ; Update ghosts
    jsr update_ghosts
    ; Log Blinky position and state (optional)
.ifdef ENABLE_STATE_TRACE
    LOG_P2 213, ghost_x+GHOST_BLINKY, ghost_y+GHOST_BLINKY
.endif
    
    ; Before collision checks, ensure queued dot/fruit processing has applied
    jsr queue_drain_all
    
    ; Check for game over conditions
    jsr check_game_over
    
    ; Check collisions (once)
    jsr check_collisions

; ==============================================================================
; COMMAND IMPLEMENTATION: CMD_07_SCORE_DISPLAY_UPDATE
; Purpose: Update score, hiscore, and status rows. Kept adjacent to UI code.
; ==============================================================================
cmd_07_score_display_update:
...
; ==============================================================================
; COMMAND IMPLEMENTATION: CMD_06_INPUT_PROCESS
; Purpose: Read inputs and update pause state (START). Kept co-located with
;          input/game loop code for clarity and easy auditing.
; ==============================================================================
cmd_06_input_process:
    jsr read_input
    lda ZP_INPUT_STATE
    and #$10
    beq :+
...
ghosts_init:
    ...
    lda #15             ; "Pacman: VERA register %1 set to %2" (placeholder init complete marker)
    sta ASM_LOG_INFO
    rts
...
@eyes_move:
    ...
    plx
    ; If computed target tile is not walkable, invoke obstacle-avoid handler for this ghost
    ; Save ghost index in A
    txa
    pha
    ; Load target into X,Y for tile read
    ldx ZP_TEMP1
    ldy ZP_TEMP2
    jsr maze_get_char_xy
    jsr is_walkable_char       ; Z=1 if walkable
    beq :+                     ; if walkable, skip avoid
    ; restore ghost index and enqueue proper handler (no immediate call)
    pla
    tax
    cpx #GHOST_BLINKY
    bne @avoid_pinky
    stx ZP_TEMP1
    lda #CMD_08_GHOST0_OBSTACLE_AVOID
    ldx ZP_TEMP1
    jsr queue_post
    bra @after_avoid
@avoid_pinky:
    ...
    bne @avoid_inky
    stx ZP_TEMP1
    lda #CMD_09_GHOST1_OBSTACLE_AVOID
    ldx ZP_TEMP1
    jsr queue_post
    bra @after_avoid
@avoid_inky:
    ...
    bne @avoid_clyde
    stx ZP_TEMP1
    lda #CMD_0A_GHOST2_OBSTACLE_AVOID
    ldx ZP_TEMP1
    jsr queue_post
    bra @after_avoid
@avoid_clyde:
    stx ZP_TEMP1
    lda #CMD_0B_GHOST3_OBSTACLE_AVOID
    ldx ZP_TEMP1
    jsr queue_post
    bra @after_avoid
:
    pla                        ; pop saved ghost index if not avoided
    tax
@after_avoid:
...
@do_move:
    ...
    jsr ghost_is_center
    bne @just_step
    ; When aligned with ROM behavior, queue optional helper commands
    ; Scatter targeting helper when in scatter
    lda ghost_state,x
    cmp #GHOSTSTATE_SCATTER
    bne @no_scatter_helper
    cpx #GHOST_BLINKY
    bne @chk_pinky_sc
    lda #CMD_0C_GHOST0_SCATTER_TARGET
    ldx #GHOST_BLINKY
    jsr queue_post
    bra @after_helper
@chk_pinky_sc:
    cpx #GHOST_PINKY
    bne @chk_inky_sc
    lda #CMD_0D_GHOST1_SCATTER_TARGET
    ldx #GHOST_PINKY
    jsr queue_post
    bra @after_helper
@chk_inky_sc:
    cpx #GHOST_INKY
    bne @chk_clyde_sc
    lda #CMD_0E_GHOST2_SCATTER_TARGET
    ldx #GHOST_INKY
    jsr queue_post
    bra @after_helper
@chk_clyde_sc:
    lda #CMD_0F_GHOST3_SCATTER_TARGET
    ldx #GHOST_CLYDE
    jsr queue_post
    bra @after_helper
@no_scatter_helper:
    ; In chase, we may need obstacle avoidance helper if target blocked (placeholder)
@after_helper:
    ; Drain queued AI helpers so their effects apply before movement
    jsr queue_drain_all
    phx
    jsr ghost_compute_target   ; ZP_TEMP1/2 = target tile (tx,ty)
    plx
...
; ==============================================================================
; COMMAND IMPLEMENTATION: CMD_0C..0F_GHOST*_SCATTER_TARGET
; Purpose: Force ZP_TEMP1/2 to scatter corner per ghost
; ==============================================================================
cmd_0C_ghost0_scatter_target:
    lda #<ghost_scatter_targets
    sta ZP_PTR1
    lda #>ghost_scatter_targets
    sta ZP_PTR1+1
    ldy #0
...
; ==============================================================================
; COMMAND IMPLEMENTATION: CMD_08..0B_GHOST*_OBSTACLE_AVOID
; Purpose: Adjust target when computed tile is blocked
; For now: if next tile toward current target is blocked, probe U,L,D,R and
; choose first walkable (caller's choose_move skips reverse)
; ==============================================================================
cmd_08_ghost0_obstacle_avoid:
    ; param in ZP_TEMP1 = ghost index
    ldx ZP_TEMP1
    ; compute current target for this ghost
    jsr ghost_compute_target
    jsr ghost_obstacle_avoid_common
    rts
cmd_09_ghost1_obstacle_avoid:
    ldx ZP_TEMP1
    jsr ghost_compute_target
    jsr ghost_obstacle_avoid_common
    rts
cmd_0A_ghost2_obstacle_avoid:
    ldx ZP_TEMP1
    jsr ghost_compute_target
    jsr ghost_obstacle_avoid_common
    rts
cmd_0B_ghost3_obstacle_avoid:
    ldx ZP_TEMP1
    jsr ghost_compute_target
    jsr ghost_obstacle_avoid_common
    rts

ghost_obstacle_avoid_common:
    ; expects ghost index in X; uses ZP_TEMP1/2 as current target
    ; try U,L,D,R from current tile; pick first walkable tile as new target
    phx
    ; load current tile
    lda ghost_x,x
    sta ZP_TEMP5
    lda ghost_y,x
    sta ZP_TEMP6
    ; dir list U,L,D,R in ZP_PTR1..ZP_PTR2
    lda #DIR_UP
    sta ZP_PTR1
    lda #DIR_LEFT
    sta ZP_PTR1+1
...
@post_move:
    ; Dot/Pill collection is handled via command queue (qh_0A)
    jmp @done
...
ui_update:
    ; Redraw scores and status fruits only (fruit control handled via queue)
    jsr update_score_digits
    jsr update_hiscore_if_needed
    jsr maybe_award_extra_life
    ; player 1 score
...
; Fruit responsibilities (spawn, tick/despawn, pickup, UI refresh) for queue handler
queue_fruit_tick:
    ; If no fruit active, maybe spawn
    lda active_fruit
    bne @tick
...
; ==============================================================================
; COMMAND IMPLEMENTATION: CMD_09_FRUIT_CONTROL
; Purpose: Spawn/tick/pickup fruit and update UI. Implementation inlined here to
;          keep the command logic in one place for readability.
; ==============================================================================
cmd_09_fruit_control:
    ; If no fruit active, maybe spawn (two per round)
    lda active_fruit
    bne @tick
    lda fruit_spawned_count
    cmp #2
...
@spawn:
    ...
    inc fruit_spawned_count
@tick:
    ; Tick fruit timer and hide on timeout
    lda active_fruit
    beq @ui
    lda fruit_timer
    ora fruit_timer+1
...
@dec_lo:
    ...
    jsr vera_disable_sprite
@pickup:
    ; Pickup when Pac-Man overlaps fruit tile
    lda active_fruit
    beq @ui
    lda pacman_x
    cmp #FRUIT_X
    ...
    jsr vera_disable_sprite
@ui:
    ; UI refresh for fruit popup/scores
    jsr popup_show_fruit
    jsr update_score_digits
    jsr update_hiscore_if_needed
    jsr maybe_award_extra_life
...
; ==============================================================================
; Pellet responsibilities for queue handler (center-only collect)
; Kept as a helper used by CMD_0A.
; ==============================================================================
queue_pellet_tick:
    ; Only when centered
    lda pac_off_x
    ...
    cmp #COLLISION_THRESH_PLUS1
    bne @ret
    ; One-shot guard per center
    lda pac_center_consumed_flag
    bne @ret
    lda #1
    sta pac_center_consumed_flag
    ; Read tile
    ldx pacman_x
    ldy pacman_y
    jsr maze_get_char_xy
...
; ==============================================================================
; COMMAND IMPLEMENTATION: CMD_0A_PELLET_CONSUMPTION
; Purpose: Center-only dot/pill collection (delegates to queue_pellet_tick)
; ==============================================================================
cmd_0A_pellet_consumption:
    jsr queue_pellet_tick
    rts
cmd_pellet_dot:
    jsr pm_eat_dot
    rts
...
apply_ghost_dot_limits_for_level:
    ; Defaults per classic behavior: Inky 30 dots, Clyde 60 dots on early levels
    ; Pinky uses timer-based release (handled elsewhere)
    lda #30
